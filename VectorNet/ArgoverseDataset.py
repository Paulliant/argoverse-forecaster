...
def get_feature(init_df: pd.DataFrame) -> pd.DataFrame:
    df = init_df.copy()
...
class ArgoverseForecastDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):   # 迭代获取数据函数，在该函数中读取了trajectory数据，同时对坐标进行了一系列预处理，最后转换为归一化的轨迹和地图tensor
        ...
        # 提取特征
        traj_df = pd.DataFrame({'X': self.trajectory[:, 1, 0], 'Y': self.trajectory[:, 1, 1], 
                                'TRACK_ID': [traj_id]*len(self.trajectory), 'OBJECT_TYPE': ['AGENT']*len(self.trajectory),
                                'TIMESTAMP': extra_fields['TIMESTAMP']})
        features_df = get_feature(traj_df)
        additional_features = features_df[['VELOCITY_X', 'VELOCITY_Y', 'MEAN_VELOCITY_X', 'MEAN_VELOCITY_Y', 'ACCELERATION_X', 'ACCELERATION_Y', 'MEAN_ACCELERATION_X', 'MEAN_ACCELERATION_Y', 
                                           'ANGULAR_MOMENTUM', 'MEAN_ANGULAR_MOMENTUM', 'NEAREST_OBSTACLE']].to_numpy()
        # additional_features = torch.from_numpy(additional_features).float()
